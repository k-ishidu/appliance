...
def read_eventteacherdata(date,nextdate,id):
    ...
    for teacherdata in teacherdata_list:
        ...
        teachervaluedata_list.append([valuedata_list,[amplitudemax,amplitudeavg],[hoursin,hourcos],opetime])
        #if id==1:
...
def make_eventtree(list,teacherdatalist):
    plt.figure()
    plt.plot(list)
    maxindex=len(list)
    ...
    for i,value in enumerate(list):
        if 50<list[i]-list[i-1]<200 or -200<list[i]-list[i-1]<-50:
            smallvariation_list.append([list[i]-list[i-1],i])
        if i!=0 and(200<list[i]-list[i-1] or list[i]-list[i-1]<-200):
            bigvariation_list.append([list[i]-list[i-1],i])
    global n
    print(bigvariation_list)
    #第一イベント
    testvariation_list=[[300,1],[500,2],[-400,5],[600,7]]
    variationtime=bigvariation_list.pop(0)
    ...
    make_eventnode(bigvariation_list,0,0,[[0,variation,0]],300,rootnode)
    
def make_eventnode(valuelist,nodenumber,appliancenumber,nowappliance_list,maxindex,nownode):
    appliance_list=copy.deepcopy(nowappliance_list)
    if len(valuelist)==0:
        print("plot")
        plot_wave(nownode,maxindex)
        return nodenumber
    vlist=copy.deepcopy(valuelist)
    variationtime=vlist.pop(0)#[値,時刻]を取得
    variation=variationtime[0]
    time=variationtime[1]
    print(variation,time)
    nodenumber+=1
    #newapplianceid=appliance_list[-1][0]+1
    for i,appliance in enumerate(appliance_list):
        if appliance[1]+variation>0 and appliance[2]!=1:
            ...
            make_eventnode(vlist,nodenumber,appliancenumber,newappliance_list,maxindex,eventnode)
...
if __name__ == '__main__':
    ...
    make_eventtree(make_smasockdatalist('2018-12-26-','2018-12-27-',800,1100),make_eventteacherdata())
