...
def make_smasockpowerlist(date,nextdate,a,b):
    ...
    '''
    newlist=[]
    for i in range(0,(len(dflist)//k)*k-k,k):
        value=0
        for j in range(0,k):
            value=+dflist[i+k]
...
def estimate(powerlist,date,mode,teacherdata):
    ...
    #list=[0,0,0,2350,2350,2350,850,850,850,700,700,700,0,0,0,0,100,100,180,150,150,0,0,0,0,0,700,700,700,700,1100,1100,0,0,0,0,1000,1000,1000,1000,1000,1000,1000,0,0,0,0]
...
def knn_appliance(square_list,date,mode,teacherdata_list):
    ...
    for teacherdata in teacherdata_list:
        if teacherdata[5]!=date:
            ...
            if teacherdata[0]-teacherdata[1]>600:
...
def make_truthdata(date,nextdate,id):
    ...
    while i<len(dfpowerlist)-1:
        power=dfpowerlist[i]-dfpowerlist[i-1]
        if power>=30:#大きい変化を見つける
            if flag==0:
                flag=1
            bigevent_list.append([power,i])
        elif -30>=power and flag==1:
            bigevent_list.append([power,i])
            if dfpowerlist[i]<600:
...
def read_eventteacherdata(date,nextdate,id,mode):
    k=30
    ...
    if mode==1:#変動小の荒い教師データ
        ...
        while i<len(newlist)-1:
            power=newlist[i]-newlist[i-1]
            if power>=30:#小さい変化を見つける
                if flag==0:
                   start=i
        ...
        if datelist[i]!=date:
            for j in range(1,12):
                if j!=4 and j!=7 and j!=8 and j!=11:
                    teacherpowerlists=read_eventteacherdata(datelist[i],datelist[i+1],j,0)
                    teacherdata_list.extend(teacherpowerlists[0])
                    if i==1 or i==3 or i==5 or i==10:
                        highteacherdata_list.extend(teacherpowerlists[1])
                if j==2 or j==4 or j==6 or j==9:
                    roughteacherpowerlists=read_eventteacherdata(datelist[i],datelist[i+1],j,1)
                    roughteacherdata_list.extend(roughteacherpowerlists[0])
                    
    '''
    newteacherdata=[]
    for i in range(0,(len(teacherdata_list)//k)*k-k,k):
        value=0
        for j in range(0,k):
            value=+teacherdata_list[j+k]
        newlist.append(value)
    '''
    return teacherdata_list,highteacherdata_list,roughteacherdata_list

    
def make_eventtree(powerlist,teacherdata_list,bigteacherdata_list,roughteacherdata_list,flag):
    plt.figure()
    plt.plot(powerlist)
    k=30
    breakflag=0
    newlist=[0]*len(powerlist)
    if flag==0:
        threshold=600
    else:
        threshold=30
    #variation_list=[[0,0],[300,1],[400,2],[-400,3],[150,4]]
    global d_list
    bigvariation_list=[]
    bigvariation_list_list=[]
    smallvariation_list=[]
    smallvariation_list_list=[]
    for i in range(2,len(powerlist)-2):
        variation=powerlist[i]-powerlist[i-1]
        if 0<variation<threshold or -threshold<variation<0:
            smallvariation_list.append([variation,i])
        if i!=0 and(threshold<variation or variation<-threshold):
            bigvariation_list.append([variation,i])
        base=min(powerlist)
        if powerlist[i]<base+5 and powerlist[i-1]<base+5 and powerlist[i+1]<base+5 and powerlist[i-2]<base+5 and powerlist[i+2]<base+5:
            bigvariation_list_list.append(bigvariation_list)
            bigvariation_list=[]
            smallvariation_list_list.append(smallvariation_list)
            smallvariation_list=[]
    bigvariation_list_list.append(bigvariation_list)
    smallvariation_list_list.append(smallvariation_list)
    global n
    ...
    slist_list=[]
    for slist in smallvariation_list_list:
        if len(slist)!=0:
            slist_list.append(slist)
        slist=[]
    #段階的な上昇および下降の統合
    for slist in slist_list:
        for svariation in slist:
            if svariation[0]>0:
                for svariation2 in slist:
                    if svariation!=svariation2 and svariation[0]+svariation2[0]>600 and svariation[1]+1==svariation2[1]:
                        newbvariation=[svariation[0]+svariation2[0],svariation2[1]]
                        if slist.index(svariation)!=0 and slist[slist.index(svariation)-1][1]==svariation[1]-1:
                            newbvariation[0]+=slist[slist.index(svariation)-1][0]
                            slist[slist.index(svariation)-1][0]=0
                        for blist in blist_list:
                            for bvariation in blist:
                                if bvariation[1]>newbvariation[1]:
                                    blist.insert(blist.index(bvariation),newbvariation)
                                    svariation[0]=0
                                    svariation2[0]=0
                                    breakflag=1
                                    break
                ...
                for blist in blist_list:
                    for bvariation in blist:
                        if abs(svariation[1]-bvariation[1])==1 and bvariation[0]>0:
                            bvariation[0]+=svariation[0]
                            svariation[0]=0
            else:
                for svariation2 in slist:
                    if svariation!=svariation2 and svariation[0]+svariation2[0]<-threshold and svariation[1]+1==svariation2[1]:
                        newbvariation=[svariation[0]+svariation2[0],svariation2[1]]
                        if slist.index(svariation)!=0 and slist[slist.index(svariation)+1][1]==svariation[1]+1:
                            newbvariation[0]+=slist[slist.index(svariation)-1][0]
                            slist[slist.index(svariation)-1][0]=0
                        for blist in blist_list:
                            for bvariation in blist:
                                if bvariation[1]>newbvariation[1]:
                                    ...
                                    breakflag=1
                                    break
                                if blist_list.index(blist)+1<len(blist_list) and blist_list[blist_list.index(blist)+1][0][1]>newbvariation[1]:
                                    blist.append(newbvariation)
                                    svariation[0]=0
                                    svariation2[0]=0
                                    breakflag=1
                                    break
    ...
    #第一イベント
    testvariation_list=[[300,1],[100,2],[-400,5],[100,7]]
    count=0
    plt.figure()
    for blist in blist_list:
        print(blist)
        ...
        finallyresult=make_eventnode(threshold,blist,0,0,[[0,variation,0]],len(powerlist),rootnode,teacherdata_list,bigteacherdata_list,[0,0])
    ...
    if flag==0:
        ...
        plt.figure()
        plt.plot(newlist)
        for i in range(0,(len(newlist)//k)*k-k,k):
            value=0
            for j in range(0,k):
                value=+newlist[i+k]
            roughlist.append(value)
        make_eventtree(roughlist,roughteacherdata_list,bigteacherdata_list,roughteacherdata_list,1)
    
def make_eventnode(threshold,valuelist,nodenumber,appliancenumber,nowappliance_list,maxindex,nownode,teacherdata_list,bigteacherdata_list,finallyresult):
    appliance_list=copy.deepcopy(nowappliance_list)
    flag=0
    if len(valuelist)==0:
        result=plot_wave(threshold,nownode,maxindex,teacherdata_list,bigteacherdata_list)
    ...
    if variation<0:
        for i in range(0,len(appliance_list)):
            for j in range(0,i):
                if appliance_list[i][2]!=1 and appliance_list[j][2]!=1 and threshold/2>appliance_list[i][1]+appliance_list[j][1]+variation>-threshold/2 and appliance_list[i][1]+variation<0 and appliance_list[j][1]+variation<0:
                    flag=1
                    ...
                    finallyresult=make_eventnode(threshold,vlist,nodenumber,appliancenumber,newappliance_list,maxindex,eventnode2,teacherdata_list,bigteacherdata_list,finallyresult)[1]
            
    
    for i,appliance in enumerate(appliance_list):
        if variation<600 and appliance[1]+variation>underlimit and appliance[2]!=1:
            flag=1
            newappliance_list=copy.deepcopy(appliance_list)
            newappliance_list[i][1]=appliance[1]+variation
            eventnode=Eventnode(nownode,appliance[0],variation,time,nodenumber)
            if appliance[1]+variation<threshold/2:
                newappliance_list[i][2]=1
            finallyresult=make_eventnode(threshold,vlist,nodenumber,appliancenumber,newappliance_list,maxindex,eventnode,teacherdata_list,bigteacherdata_list,finallyresult)[1]
    ...
    #イベント作成が不可能になった時
        ...
        if flag==0:
            newappliance_list=copy.deepcopy(appliance_list)
            eventnode=Eventnode(nownode,appliance[0],0,time,nodenumber)
            finallyresult=make_eventnode(threshold,vlist,nodenumber,appliancenumber,newappliance_list,maxindex,eventnode,teacherdata_list,bigteacherdata_list,finallyresult)[1]
            finallyresult[0]-=100
        #for appliance in appliance_list:
            #if appliance[2]==0:
                #return nodenumber,[-100,[],[]]
        #result=plot_wave(nownode,maxindex,teacherdata_list,bigteacherdata_list)
        #result[0]-=200#途中で終わったペナルティ
        #if finallyresult[0]<result[0]:
            #finallyresult=result
        return nodenumber,finallyresult

    
def plot_wave(threshold,node,maxindex,teacherdata_list,bigteacherdata_list):
    if threshold==30:
        knn=1
    else:
        knn=5
    ...
    while len(newevent_list)>0:
        ...
        if len(plotevent_list)!=0:
            ...
            if len(evaluate_list)!=0:
                evaluation=customknn(teacherdata_list,bigteacherdata_list,evaluate_list,start,knn)
...
def customknn(teacherdata_list,bigteacherdata_list,vec,starttime,k):
    ...
    for teacher in teacherdata_list:
        if opetime>100 or abs(opetime-teacher[3])<100:
            simA=min(amplitudemax,teacher[1][0])/max(amplitudemax,teacher[1][0])*WAmax
...
def evaluation(resultdata,truthdata):
    data=make_smasockpowerlist(date,nextdate,0,4000)
    group1flag=0
    group2flag=0
    
    
    appliances_list=["電子レンジ","テレビ","電気ポット","洗濯機","ドライヤー","ストーブ","加湿器","オーブン"]
    appliancesrecall_list=[0]*8
    appliancesprecision_list=[0]*8
    appliancestp_list=[0]*8
    appliancesfp_list=[0]*8
    appliancesfn_list=[0]*8
    group1=["電子レンジ","電気ポット","オーブン",]
    group1recall=0
    group1precision=0
    group1tp=0
    group1fp=0
...
if __name__ == '__main__':
    ...
    #homenumber='0030'
    date='2019-01-08-'
    nextdate='2019-01-09-'    #teacherdata=make_teacherdata()
    ...
    #make_smasockpowerlist("2018-12-30-","2018-12-31-",3100,4000)
    print(date)
    smadata=make_smasockpowerlist(date,nextdate,0,4000)
    teacherdata=make_eventteacherdata(date)
            
    truthcount=0
    for j in range(1,12):
        if j!=7 and j!=8 and j!=11:
            truth=make_truthdata(date,nextdate,j)
            if len(truth)!=0:
                for data in truth:
                    print(data)
                truthcount+=len(truth)
    print("truthcount",truthcount)
    print("以下推定結果")
    make_eventtree(smadata,teacherdata[0],teacherdata[1],teacherdata[2],0)